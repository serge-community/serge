...
Based on each configuration file's B<sync> section, pull updated translation
files from remote translation server into the local directory.
...
If not specified, all the available languages are supposed to be synchronized.
